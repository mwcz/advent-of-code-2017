...
impl Mem {
    ...
    fn position_max_first(&self) -> (usize, u8) {
        // custom max function, since the std max functions (iter().max(), iter().position_max(),
        // etc) all break ties with the last occurrence, not the first
...
fn solve(mut mem: Mem) -> usize {
    let mut seen = FnvIndexMap::<MemBanks, usize, SEEN_CAPACITY>::new();

    // mark initial layout as "seen"
    seen.insert(mem.banks.clone(), 0).unwrap();
    ...
    loop {
        ...
        if let Ok(new_opt) = seen.insert(mem.banks.clone(), seen.len()) {
            // break if the value was seen before
            if let Some(first_sighting) = new_opt {
                return seen.len() - first_sighting;
            }
        }
    }
}
...
// #[test]
// fn real_answer_test() {
//     let mem = Mem::from("0 2 7 0");
//     assert_eq!(solve(mem), 4);
// }
